...
describe "CdnZip" do
  ...
  it "sync" do
    execute "rake cdn_zip_PREFIX=#{@prefix} assets:precompile"
  ...
  it "sync with enabled=false" do
    execute "rake cdn_zip_PREFIX=#{@prefix} cdn_zip_ENABLED=false assets:precompile"
    bucket(@prefix).files.size.should == 0
  end

  it "sync with gzip_compression=true" do
    execute "rake cdn_zip_PREFIX=#{@prefix} cdn_zip_GZIP_COMPRESSION=true assets:precompile"
